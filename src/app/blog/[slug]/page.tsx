            ...
            {post.tags.length > 0 && (
              <div className="flex gap-2 mb-4">
                {post.tags.map((tag: string) => (
                  <span key={tag} className="font-sans text-xs tracking-widest uppercase text-fuchsia-300">{tag}</span>
                ))}
              </div>
            )}
            <h1 className="font-display text-3xl md:text-5xl font-400 text-white leading-tight">{post.title}</h1>
